<?php
    ...
    if(isset($_POST['dat']))
    {
        ...
        $sql_query = "SELECT id, firstname, lastname,username FROM {user} WHERE username = '".$_POST['dat']."';";
        $info_user = $DB->get_record_sql($sql_query);
    
        if($info_user){
            $sql_query = "SELECT nombre_rol,id_jefe FROM {talentospilos_user_rol} INNER JOIN {talentospilos_rol} ON {talentospilos_user_rol}.id_rol = {talentospilos_rol}.id   WHERE {talentospilos_user_rol}.id_usuario = '".$info_user->id."' AND {talentospilos_user_rol}.estado = 1 AND {talentospilos_user_rol}.id_semestre = (SELECT MAX(id)  FROM {talentospilos_semestre});";
        ...
        echo json_encode(getProfessionals());
    }
?>